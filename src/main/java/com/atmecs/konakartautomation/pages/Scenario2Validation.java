...
public class Scenario2Validation {
	...
	public static void sortingDateValidation(String sortingoption, Logger log) {
		...
		for (int index = 1; index < loopCount; index++) {
			...
			if (sortingoption.equalsIgnoreCase(CommonUtility.getElement(LoadPropertyFile.locators.getProperty("loc.konakart.sortbydatemostrecent.txt")).getText())) {
			...
			} else if (sortingoption.equalsIgnoreCase(CommonUtility.getElement(LoadPropertyFile.locators.getProperty("loc.konakart.sortbydateoldest.txt")).getText())) {
				Assert.assertTrue(difference <= 0, "Sorting Functionality Failed");
				ExtentReport.messagePrint("Sorting by date Functionality Working Properly");
				log.info("Sorting by date Functionality Working Properly");
			}
	...
	public static void heroImgSortOption(String sortingoption) {
		...
		//CommonUtility.explicitWait(LoadPropertyFile.locators.getProperty("loc.konakart.heroimgtitle.txt"), 30);
	}
	...
	public static void starRatingValidation(String sortingoption, Logger log) {
		...
		for (int index = 1; index < loopCount; index++) {
			...
			if (sortingoption.equalsIgnoreCase(CommonUtility.getElement(LoadPropertyFile.locators.getProperty("loc.konakart.sortbyratinghightolow.txt")).getText())) {
				Assert.assertTrue(difference >= 0, "Sorting Functionality Failed");
			...
			} else if (sortingoption.equalsIgnoreCase(CommonUtility.getElement(LoadPropertyFile.locators.getProperty("loc.konakart.sortbyratinglowtohigh.txt")).getText())) {
				Assert.assertTrue(difference <= 0, "Sorting Functionality Failed");
				ExtentReport.messagePrint("Sorting by rating Functionality Working Properly");
				log.info("Sorting by rating Functionality Working Properly");
			}
